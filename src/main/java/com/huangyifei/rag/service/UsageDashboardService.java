...
public class UsageDashboardService {
    ...
    public UsageOverview buildOverview(int days) {
        ...
        List<String> userIds = users.stream().map(user -> String.valueOf(user.getId())).toList();
        ...
        DailyUsagePoint today = trends.isEmpty() ? new DailyUsagePoint("", 0, 0, 0, 0, 0) : trends.get(trends.size() - 1);
        ...
        List<UsageAlert> alerts = users.stream()
                ...
                .sorted(Comparator.comparing((UsageAlert alert) -> alert.level().equals("critical") ? 0 : 1)
                        .thenComparing(UsageAlert::usageRatio)
                        .reversed())
                .toList();
    ...
    private UsageAlert buildAlert(User user, String scope, UsageQuotaService.QuotaView quota) {
        ...
        double ratio = (double) quota.usedTokens() / quota.limitTokens();
        if (quota.remainingTokens() == 0) {
            return new UsageAlert("critical", String.valueOf(user.getId()), user.getUsername(), scope,
                    quota.usedTokens(), quota.limitTokens(), quota.remainingTokens(), quota.requestCount(), ratio, "额度已用尽");
        }
        if (ratio >= 0.8d) {
            return new UsageAlert("warning", String.valueOf(user.getId()), user.getUsername(), scope,
                    quota.usedTokens(), quota.limitTokens(), quota.remainingTokens(), quota.requestCount(), ratio, "额度即将用尽");
        }
        return null;
    }
