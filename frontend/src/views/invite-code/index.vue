...
import { NButton, NInput, NInputNumber, NPopconfirm, NSwitch, NTag } from 'naive-ui';
...
import {
  ...
  fetchGetRegistrationSettings,
  fetchUpdateInviteCode,
  fetchUpdateRegistrationSettings
} from '@/service/api';
...
const registrationSettings = ref<Api.InviteCode.RegistrationSettings | null>(null);
...
const inviteRequired = computed(() => registrationSettings.value?.inviteRequired ?? false);
...
async function getRegistrationSettings() {
  ...
  const { data: settings, error } = await fetchGetRegistrationSettings();
...
async function handleInviteRequiredChange(value: boolean) {
  settingsLoading.value = true;
  const { data: settings, error } = await fetchUpdateRegistrationSettings({ inviteRequired: value });
  settingsLoading.value = false;
  ...
  window.$message?.success(value ? '已开启邀请码注册' : '已关闭邀请码注册');
}
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-auto">
    ...
    >
      <div class="mb-4 border border-#d8e8ff rounded-16px bg-#f4f9ff px-18px py-14px">
        <div class="flex flex-wrap items-center justify-between gap-12px">
          <div>
            <div class="text-15px text-#1f2937 font-semibold">注册方式控制</div>
            <div class="mt-1 text-13px text-#667085">
              开启后，新用户注册必须填写有效邀请码；通过邀请码注册会自动赠送
              {{ registrationSettings?.inviteGiftLlmTokens ?? 0 }} LLM Token 和
              {{ registrationSettings?.inviteGiftEmbeddingTokens ?? 0 }} Embedding Token。
            </div>
          </div>
          <NSwitch
            ...
            checked-text="需要邀请码"
            unchecked-text="开放注册"
            @update:value="handleInviteRequiredChange"
          />
        </div>
      </div>
