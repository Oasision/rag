...
const createOrder = async () => {
  ...
  try {
    ...
    if (!error && data) {
      // LOCAL SELF TEST: the backend marks the new recharge order as paid immediately.
      // Restore the commented real WeChat Pay modal flow when real payment is needed.
      /*
      // Real WeChat Pay: show scan-to-pay modal.
      ...
      */
      ...
      await getOrders();
      window.$message?.success('本地自测充值成功，余额已到账');
    }
  } finally {
