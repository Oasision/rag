...
import { uploadAccept, uploadIconExtensions } from '@/constants/common';
...
function renderIcon(fileName: string) {
  ...
  if (ext) {
    ...
    const iconName = uploadIconExtensions.includes(normalizedExt) ? normalizedExt : 'dflt';
    return (
      <span
        class="mr-12px inline-flex shrink-0 items-center justify-center"
        style="width: 50px; height: 50px; font-size: 24px;"
      >
        <SvgIcon localIcon={iconName} class="h-full w-full" />
      </span>
    );
  }
  return null;
...
async function onBeforeUpload(
  ...
  if (!error) {
    const uploadedChunks = progress.uploadedChunks ?? progress.uploaded ?? [];
    row.file = options.file.file!;
    row.status = UploadStatus.Pending;
    row.progress =
      progress.progress ?? Number.parseFloat(((uploadedChunks.length / Math.ceil(row.totalSize / chunkSize)) * 100).toFixed(2));
    row.uploadedChunks = uploadedChunks;
    store.startUpload();
    loading.value = false;
    return true;
  }
