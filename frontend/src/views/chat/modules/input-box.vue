...
const { connectionStatus, conversationId, input, isRateLimited, list, rateLimitRemainingSeconds, wsData } =
...
function handleStartPayload(assistant: Api.Chat.Message, payload: Record<string, any>) {
  ...
  for (let i = list.value.length - 1; i >= 0; i -= 1) {
    const item = list.value[i];
    if (item?.role === 'user' && !item.conversationId) {
      item.conversationId = conversationId.value;
      item.generationId = assistant.generationId;
      break;
    }
  }
}
...
watch(wsData, val => {
  ...
  if (payload.type === 'completion') {
    ...
    chatStore.loadSessions(false).catch(() => {});
    return;
  }
