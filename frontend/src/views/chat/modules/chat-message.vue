...
function createBracketCitationLink(sourceNum: string): string {
  const referenceNumber = parseInt(sourceNum, 10);
  const detail = props.msg.referenceMappings?.[String(referenceNumber)] || props.msg.referenceMappings?.[referenceNumber];

  if (!detail?.fileName && !detail?.fileMd5) {
    return `[${sourceNum}]`;
  }

  const linkClass = 'source-file-link citation-source-link';
  const fileId = `source-file-${sourceFiles.value.length}`;
  const fileName = detail.fileName || `来源#${sourceNum}`;
  ...
  sourceFiles.value.push({
    ...
    pageNumber: detail.pageNumber ?? undefined
  });

  const pageLabel = detail.pageNumber ? ` (第${detail.pageNumber}页)` : '';
  return `(来源#${sourceNum}: <span class="${linkClass}" data-file-id="${fileId}" title="${escapeHtmlAttribute(fileName)}">${fileName}${pageLabel}</span>)`;
}
...
function processSourceLinks(text: string): string {
  ...
  processedText = processedText.replace(/\[(\d+)\]/g, (match, sourceNum, offset: number, source: string) => {
    const previousChar = source[offset - 1] || '';
    ...
    return createBracketCitationLink(sourceNum);
  });
...
function extractContextAnchorText(target: HTMLElement) {
  ...
  const beforeCitation = rawText.split(/(?:\(|（)?来源#\d+:|\[\d+\]/)[0] || rawText;
...
<template>
  <div class="mb-8 flex-col gap-2">
    ...
    <div v-else-if="msg.role === 'assistant'" class="chat-markdown mt-2 pl-12" @click="handleContentClick">
