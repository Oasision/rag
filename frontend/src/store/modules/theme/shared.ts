...
export function initThemeSettings() {
  ...
  if (!isProd) return normalizeThemeLayoutMode(themeSettings);
  ...
  return normalizeThemeLayoutMode(settings);
}

function normalizeThemeLayoutMode(settings: App.Theme.ThemeSetting) {
  const unsupportedModes: UnionKey.ThemeLayoutMode[] = ['vertical-mix', 'horizontal', 'horizontal-mix'];

  if (unsupportedModes.includes(settings.layout.mode)) {
    settings.layout.mode = 'vertical';
    settings.layout.reverseHorizontalMix = false;
  }
...
export function getNaiveTheme(colors: App.Theme.ThemeColor, recommended = false) {
  ...
  const theme: GlobalThemeOverrides = {
    common: {
      ...
      boxShadow1: '0 10px 28px rgb(15 23 42 / 0.08)',
      boxShadow2: '0 18px 42px rgb(15 23 42 / 0.1)',
      boxShadow3: '0 24px 56px rgb(15 23 42 / 0.12)'
    },
    Button: {
