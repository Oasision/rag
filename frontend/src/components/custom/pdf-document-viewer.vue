  ...
  <div class="pdf-viewer-shell">
    <div v-if="!embeddedHeader" class="pdf-viewer-toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <NButton size="tiny" quaternary :disabled="zoom <= minZoom" @click.stop="zoomOut">
          <template #icon>
            <icon-mdi-magnify-minus-outline />
          </template>
        </NButton>
        <NButton size="tiny" quaternary :disabled="zoom >= maxZoom" @click.stop="zoomIn">
          <template #icon>
            <icon-mdi-magnify-plus-outline />
          </template>
        </NButton>
        <NButton size="tiny" secondary @click.stop="resetZoom">适应宽度</NButton>
...
function resetZoom() {
  setZoom(1, { forceRenderWhenSame: true });
}

function setZoom(value: number, options?: { forceRenderWhenSame?: boolean }) {
  const nextZoom = Math.min(Math.max(Number(value.toFixed(2)), minZoom), maxZoom);
  lastSuccessfulRenderSignature = '';

  if (nextZoom === zoom.value) {
    if (options?.forceRenderWhenSame && pdfDocument.value) {
      void forceRender();
    }
    return;
  }
